...
/* === UAPI definitions (from <uapi/linux/ntsync.h>) === */
...
#define NTSYNC_WAIT_REALTIME	0x01
			 ...
			 struct ntsync_obj *locked_obj)
{
	...
	/* 4.14: use atomic_cmpxchg instead of atomic_try_cmpxchg */
...
static int release_sem_state(struct ntsync_obj *sem, __u32 count)
{
	...
	/* Manual overflow check for 4.14 */
	sum = sem->u.sem.count + count;
	if (sum < sem->u.sem.count || sum > sem->u.sem.max)
		return -EOVERFLOW;
					   ...
					   enum ntsync_type type)
{
	...
	/* 4.14: kzalloc_obj not available */
...
{
	/* 4.14: manual fd allocation instead of FD_PREPARE */
	struct file *file;
	int fd;
...
static struct ntsync_obj *get_obj(struct ntsync_device *dev, int fd)
{
	...
	/* 4.14: keep file reference for put_obj */
	obj->file = file;
	return obj;
}
...
/* 4.14: coarse schedule_timeout instead of hrtimer */
static int ntsync_schedule(const struct ntsync_q *q, const struct ntsync_wait_args *args)
{
	...
	if (args->timeout == U64_MAX) {
		...
		/* Convert nanoseconds to jiffies (coarse) */
		      ...
		      struct ntsync_q **ret_q)
{
	...
	/* 4.14: manual size check instead of array_size */
	...
	/* 4.14: manual overflow check */
	if (count > NTSYNC_MAX_WAIT_COUNT || size > sizeof(fds))
		return -EINVAL;
	...
	/* 4.14: u64_to_user_ptr not available */
	...
	/* 4.14: kmalloc_flex not available */
...
static int ntsync_wait_all(struct ntsync_device *dev, void __user *argp)
{
	...
	/* check alert event after normal objects */
	if (args.alert) {
